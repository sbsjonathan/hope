export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (!targetUrl) {
        targetUrl = "https://www.jw.org/pt/biblioteca/revistas/sentinela-estudo-janeiro-2026/";
      }
...
async function getFallbackUrlFromTOC(issue, studyNumber) {
  ...
  if (idx >= 0 && idx < matches.length) {
    return `https://www.jw.org` + matches[idx];
  }
  return null;
}
...
function PROCESSADOR_5(html) {
  ...
  out = out.replace(
    /<div\b[^>]*\bid=(?:"|')tt11(?:"|')[^>]*>[\s\S]*?<p\b[^>]*>[\s\S]*?<strong[^>]*>\s*OBJETIVO\s*<\/strong>([\s\S]*?)<\/p>[\s\S]*?<\/div>/gi,
...
function PROCESSADOR_6(html) {
  ...
  out = out.replace(
    ...
    (m, id, qClass, qLg, src) => {
      // Simplificado por segurança para capturar a imagem
      if (!src) return m;
...
function PROCESSADOR_7(html) {
  ...
  out = out.replace(
    ...
    (m) => m // Preservado para não quebrar
  );
  out = out.replace(
